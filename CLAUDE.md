...
- **Trigger-entry lifecycle**: `handle_open` rests a trigger-market order at `ENTRY_IMPROVEMENT` (0.5%) beyond the target's entry price in our favor (long: below, short: above) and stores its oid in the entry's `entry_oid`; `mirrored` stays False until the trigger fills. Exception: when the current price is already at/past the trigger (Hyperliquid rejects triggers that would fire immediately), the position is opened at market right away and TP/SL placed immediately — this is also how pre-existing target positions that are already underwater get mirrored at startup. On startup, `retry_unmirrored` drops unmirrored entries with no resting trigger from state so the first poll replays them through `handle_open` (one retry per restart; every guard re-applies). Fills are detected two ways: the `userFills` websocket (`handle_fill`, usually within ~1s) and the poll-time sweep `check_pending_entries` as backstop. Either path *promotes* the entry — records `hl_size` from the actual exchange position, sets `mirrored: True`, clears `entry_oid`, and only then places TP/SL (reduce-only orders need a position). If the price never reaches the trigger, the order rests until the target closes the trade, then `handle_close` cancels it — the trade is simply never entered. An oid that vanishes without a fill (cancelled externally) is warned once and never retried.
...
- Risk controls (phase 3) gate every exposure-adding order: a new trade's margin is `min(bot_capital / MAX_OPEN_TRADES, free_margin)` (`allocate_margin`), its notional is clamped by `MAX_POSITION_NOTIONAL_USD` and `MAX_ACCOUNT_LEVERAGE`, and it's rejected (`mirrored: False`) when the surviving notional is below `MIN_NOTIONAL_USD`. There is no daily loss limit (removed by decision 2026-07-11); a stale `state["risk"]` key may linger in old state files and is ignored.
- **Bot capital comes from the exchange's available balance.** `margin_summary` reads the unified account's spendable USDC (spot total minus `hold` — exactly the UI's "Available balance"; the exchange deducts everything committed to positions and resting orders via `hold`). Foreign positions ("foreign" = exchange positions minus tracked mirrored coins, recomputed at every open) therefore cost the bot nothing extra; `adjust_for_foreign` adds the bot's *own* positions' margin back so the per-trade slice (`capital / MAX_OPEN_TRADES`) stays a share of all bot money, and strips foreign notional from the `MAX_ACCOUNT_LEVERAGE` input. Foreign positions must use isolated margin, and the bot's own positions are opened isolated too (`is_cross=False` in `update_leverage`), so every trade's risk is contained to its own bucket. At startup (live mode) `confirm_foreign_positions` prints the bot's trading capital, lists foreign positions and always asks for interactive y/N confirmation (even with none foreign) — declining exits before the first poll. A pending entry whose trigger fired while the bot was down (position matches the entry's direction) counts as ours there, not foreign — the first poll promotes it.
