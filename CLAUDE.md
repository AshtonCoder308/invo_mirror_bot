...
- **Foreign positions are excluded from the bot's capital.** "Foreign" = exchange positions minus tracked mirrored coins, recomputed at every open. `adjust_for_foreign` strips their margin and notional from the sizing inputs so the bot only trades what isn't theirs. Foreign positions **must use isolated margin** — an isolated position's `marginUsed` is its own margin balance and absorbs its uPnL, so subtracting it removes both the capital and the PnL swings; a cross foreign position would break this math. At startup (live mode) `confirm_foreign_positions` prints the bot's trading capital, lists foreign positions and always asks for interactive y/N confirmation (even with none foreign) — declining exits before the first poll. A pending entry whose trigger fired while the bot was down (position matches the entry's direction) counts as ours there, not foreign — the first poll promotes it.
